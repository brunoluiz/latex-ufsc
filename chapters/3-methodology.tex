  ...
  \label{methodology:structure:sw}
    ...
    Devido a complexidade do projeto de software e suas diversas classes e implementações, a lista abaixo resume as principais classes que permitem o funcionamento da estação. Essas serão referidas nas seguintes seções sobre o funcionamento da estação. 
        ...
        \subsubsection{AuthHandler}
          Gerencia autenticação via RFID. Primeiramente checa se o usuário está habilitado por meio de uma \textit{whitelist} e, caso não encontre nenhum dado nela, faz uma requisição ao servidor OCPP para a verificação.
        \subsubsection{OcppManager}
          Roda em uma instância própria e, a cada período de tempo, executa a lista de comandos que foram criados pelo programa. Implementa o \textit{Dispatcher} do \textit{Command Pattern} \cite{book-gof}, onde os seus comandos são implementações das requisições da tabela \ref{table:ocpp}.
        \subsubsection{ChargePointServer}
          Implementação da classe WSDL gerada pelo wsimport. Roda em uma instância própria e cria um servidor ao qual o \textit{Central System} pode requisitar dados.
        \subsubsection{ChargePointClient}
          Implementação da classe WSDL gerada pelo wsimport. Permite enviar requisições para o \textit{Central System} e é chamado apenas por comandos despachados pelo OcppManager.
        ...
        \subsubsection{ConnectorManager e Connector}
          Classe que armazena todos conectores, estes instancias da classe Connector. As classes \textit{Connector} gerenciam cada conector da estação, o que possibilita a obtenção de dados e controle por meio de diferentes interfaces (\ac{IO} e MODBUS no momento).
        \subsubsection{TransactionManager e Transaction}
          Classe que armazena transações ativas, sendo que transações são instancias da classe Transaction. Cada transação roda em uma instância individual, sendo que estas são responsáveis pelos processos de inicialização e finalização junto ao OCPP, assim como envio periódico de dados da transação.

    \subsection{Inicialização}

      A figura \ref{fig:sw-init} apresenta como o programa é inicializado. Deve se levar em conta que a primeira coisa a ser executada, antes mesmo da sequência definida no \textit{main()}, é a inicialização do Guice. Ele faz todos \textit{check-ups} iniciais para ver se não há nenhuma declaração errada na sua configuração.
      ...
      \subsection {Inicialização e Finalização de Transações}
